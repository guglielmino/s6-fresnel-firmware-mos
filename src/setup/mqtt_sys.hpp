...
void mqtt_sys_init() {
    ...
    makeDeviceTopic(pubSensPowerTopic, MAX_TOPIC_LEN, PUB_SENS_POWER_TOPIC, settings.s6fresnel().gateway(),
                    settings.s6fresnel().group(), settings.deviceId());
    makeDeviceTopic(subSwitchDevTopic, MAX_TOPIC_LEN, SUB_SWITCH_DEV, settings.s6fresnel().gateway(),
                    settings.s6fresnel().group(), settings.deviceId());
    makeRoomTopic(subSwitchRoomTopic, MAX_TOPIC_LEN, SUB_SWITCH_ROOM, settings.s6fresnel().gateway(),
                  settings.s6fresnel().group());

    makeDeviceTopic(pubInfoTopic, MAX_TOPIC_LEN, PUB_SENS_INFO_TOPIC, settings.s6fresnel().gateway(), settings.s6fresnel().group(),
                    settings.deviceId());

    makeDeviceTopic(pubSensDailyKwhTopic, MAX_TOPIC_LEN, PUB_SENS_DAILYKWH_TOPIC, settings.s6fresnel().gateway(),
                    settings.s6fresnel().group(), settings.deviceId());

    makeDeviceTopic(pubPowerFeedbackTopic, MAX_TOPIC_LEN, PUB_EVENT_POWERFEEDBACK_TOPIC, settings.s6fresnel().gateway(),
                    settings.s6fresnel().group(), settings.deviceId());

    makeDeviceTopic(pubLWTTopic, MAX_TOPIC_LEN, PUB_EVENT_LWT_TOPIC, settings.s6fresnel().gateway(),
                    settings.s6fresnel().group(), settings.deviceId());

    makeDeviceTopic(pubSensCurrentTopic, MAX_TOPIC_LEN, PUB_SENS_CURRENT_TOPIC, settings.s6fresnel().gateway(),
                    settings.s6fresnel().group(), settings.deviceId());

    makeDeviceTopic(pubSensFreqTopic, MAX_TOPIC_LEN, PUB_SENS_FREQUENCY_TOPIC, settings.s6fresnel().gateway(),
                    settings.s6fresnel().group(), settings.deviceId());

    makeDeviceTopic(pubSensReactivePowerTopic, MAX_TOPIC_LEN, PUB_SENS_RPOWER_TOPIC, settings.s6fresnel().gateway(),
                    settings.s6fresnel().group(), settings.deviceId());

    makeDeviceTopic(pubSensPowerFactorTopic, MAX_TOPIC_LEN, PUB_SENS_POWERFACTOR_TOPIC, settings.s6fresnel().gateway(),
                    settings.s6fresnel().group(), settings.deviceId());

    makeDeviceTopic(pubSensVoltageTopic, MAX_TOPIC_LEN, PUB_SENS_VOLTAGE_TOPIC, settings.s6fresnel().gateway(),
                    settings.s6fresnel().group(), settings.deviceId());

    makeDeviceTopic(pubEventCrontabTopic, MAX_TOPIC_LEN, PUB_EVENT_CRONTAB_TOPIC, settings.s6fresnel().gateway(),
                    settings.s6fresnel().group(), settings.deviceId());
